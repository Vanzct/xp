...
def set_expire():
    ...
    redis_client.expire(TOKEN, TOKEN_EXPIRE_SECONDS)  # 两小时内10次短信验证
...
class RedisClient():
    ...
    def set_user_id(value):
        return redis_client.set(USER_ID, value)
    ...
    @staticmethod
    def set_wx_jsapi_ticket(token, s):
        redis_client.set("wx_jsapi_ticket", token)
        redis_client.expire("wx_jsapi_ticket", s)

    @staticmethod
    def set_wx_access_token(token, s):
        redis_client.set("wx_access_token", token)
        redis_client.expire("wx_access_token", s)

    @staticmethod
    def get_access_token():
        return redis_client.get("wx_access_token")